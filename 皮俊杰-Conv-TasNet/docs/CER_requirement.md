CER 字符错误率（Character Error Rate）标准公式
 
CER = (S + I + D) / N
 
符号说明
 
- S：Substitution 替换错误：字符认错，标准答案是A，识别成B
- I：Insertion 插入错误：多出来的字符（模型凭空多输出字）
- D：Deletion 删除错误：标准答案有的字，模型漏输出
- N：参考文本总字符数（Ground Truth 长度）
 
和 WER 区别
 
- CER：按单个字符算（中文ASR/OCR通用）
- WER：按单词算（英文）
 
最简计算步骤（Levenshtein 编辑距离）
 
1. 求 GT 和 Hyp 的编辑距离（最少替换+插入+删除步数）
2. 编辑距离 ÷ GT字符总数 = CER
