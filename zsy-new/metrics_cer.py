...
def normalize_text(text: str | None) -> str:
    """Official-style text normalization before CER/RR evaluation."""
    if text is None:
        return ""
...
def is_correct_rejection(label: str, content: str) -> bool:
    return is_rejection_sample(label) and normalize_text(content) == ""


def is_correct_acceptance(label: str, content: str) -> bool:
    return not is_rejection_sample(label) and normalize_text(content) != ""
...
class EvalSummary:
    ...
    def final_cer(self) -> float:
        ...
        if not pos:
            return 0.0
        refs = sum(len(normalize_text(r.label)) for r in pos)
        if refs == 0:
