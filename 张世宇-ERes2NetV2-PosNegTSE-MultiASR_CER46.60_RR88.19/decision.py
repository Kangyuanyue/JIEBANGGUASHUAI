...
def final_decision(e: DecisionEvidence, cfg) -> DecisionResult:
    ...
    min_command_prior = getattr(cfg, "min_command_prior_for_accept", 0.0)
    strict_below_similarity = getattr(cfg, "command_prior_strict_below_similarity", 1.0)
    if (
        e.asr_confidence >= cfg.min_asr_confidence
        and e.command_prior_score < min_command_prior
        and e.speaker_similarity < strict_below_similarity
    ):
        return DecisionResult(False, score, "low_command_prior", use_tse=use_tse)
